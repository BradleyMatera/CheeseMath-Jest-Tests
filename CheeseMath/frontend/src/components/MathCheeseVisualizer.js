...
import { addCheese, subtractCheese, multiplyCheese, divideCheese, cheeseSqrt, maxCheese } from '../utils/mathFunctions';

const MathCheeseVisualizer = () => {
  ...
  useEffect(() => {
    ...
    anime({
      targets: '.math-vis .calculator-display',
      opacity: [0, 1],
      scale: [0.95, 1],
      easing: 'easeOutQuad',
      duration: 800,
    ...
    anime({
      targets: '.math-vis .button-grid button',
      translateY: [50, 0],
      opacity: [0, 1],
  ...
  const handleOperation = (operation) => {
    try {
      ...
      if (isNaN(num1) || (value2 && isNaN(num2))) {
        setResult('ERROR');
        return;
      }
      let operationResult;
  ...
  return (
    <div className="math-vis component-container">
      <h1 style={{ color: '#ccc' }}>Math Visualizer</h1>
...
export default MathCheeseVisualizer;